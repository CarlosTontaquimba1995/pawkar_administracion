...
const EncuentrosRegisterForm: React.FC<EncuentrosRegisterFormProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = {
        ...formData,
        fechaHora: formData.fechaHora.toISOString(),
      };

      const response = await encuentroService.createEncuentro(data, token);

      if (response.success) {
        onSuccess();
        onClose();
      } else {
        throw new Error(response.message || "Error al crear el encuentro");
      }
    } catch (error: any) {
      console.error("Error creating encuentro:", error);
      setError(error.response?.data?.message || "Error al crear el encuentro");
    } finally {
      setLoading(false);
    }
  };
